...
REGULAR_STRING:                      '"'  (~["\\\r\n\u0085\u2028\u2029] | CommonCharacter)* '"';
VERBATIUM_STRING:                    '@"' (~'"' | '""')* '"';
INTERPOLATED_REGULAR_STRING_START:   '$"' -> pushMode(INTERPOLATION_STRING); //{ this.OnInterpolatedRegularStringStart(); }
INTERPOLATED_VERBATIUM_STRING_START: '$@"'  -> pushMode(INTERPOLATION_STRING); //  { this.OnInterpolatedVerbatiumStringStart(); }
...
DOUBLE_CURLY_INSIDE:           '{{';
OPEN_BRACE_INSIDE:             '{' -> skip, pushMode(DEFAULT_MODE); // { this.OpenBraceInside(); }
REGULAR_CHAR_INSIDE:           SimpleEscapeSequence; //{ this.IsRegularCharInside() }?
VERBATIUM_DOUBLE_QUOTE_INSIDE: '""'; //{ this.IsVerbatiumDoubleQuoteInside() }?
DOUBLE_QUOTE_INSIDE:           '"' -> popMode; //{ this.OnDoubleQuoteInside(); } 
...
CLOSE_BRACE_INSIDE:             '}'  -> skip, popMode; //{ this.OnCloseBraceInside(); }
FORMAT_STRING:                  ~'}'+;
